    ...
    for i, filepath in enumerate(files):
        ...
        if mode == "Docker (Рекомендуется)":
            cmd = [
                ...
                f"mkdir -p /tmp/matplotlib && cd /input && antismash '{filename}' --output-dir /output/{file_base}"
            ]
            ...
            if current_os == "Windows":
                ...
                full_cmd = f"{local_cmd} '{final_in}' --output-dir '{final_out}'"
...
def main():
    ...
    with tabs[0]:
        ...
        with col_a:
            ...
            st.caption("Для сырых геномов (FASTA/GBK).")
            if st.button("🧬 Запустить antiSMASH", disabled=not has_raw_genomes, use_container_width=True):
                with st.spinner("Выполнение antiSMASH..."):
                    ...
                    if success:
                        ...
                        st.info("Теперь нажмите **Шаг B**, чтобы запустить Nerpa.")

        # КНОПКА ШАГ B
        with col_b:
            st.markdown("### Шаг B: Nerpa")
            st.caption("Сопоставление BGC со структурами.")
    ...
    with tabs[1]:
        ...
        if st.session_state.analysis_result and st.session_state.analysis_result.success:
            ...
            st.info("👈 Сначала запустите анализ на вкладке «Данные и Запуск»")
